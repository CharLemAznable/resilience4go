...
func TestDecorateFunction(t *testing.T) {
	...
	decoratedFn1 := fallback.DecorateCheckedApplyDefault(fn1, fallback1)
	...
	decoratedFn2 := fallback.DecorateCheckedApplyDefault(fn2, fallback2)
	...
	decoratedFn3 := fallback.DecorateCheckedApplyByType[string, string, *TargetError](fn3, fallback3)
	...
	fallback4 := func(_ string, _ string, _ *TargetError) (string, error) {
		return "", errors.New("fallback error")
	}
	decoratedFn4 := fallback.DecorateCheckedApplyWithFailure(fn4, fallback4)
	...
	decoratedFn6 := fallback.DecorateCheckedApplyDefault(fn6, fallback6)
	func() {
		defer func() {
			if r := recover(); r != nil {
				if r != "original error" {
